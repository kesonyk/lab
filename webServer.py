...
def set_dist():
	...
	if request.method=='POST':
		if not session.get('logged_in'):
			abort(401)
		db=get_db()
		db.execute('UPDATE DISTTAB SET DIST=? WHERE GATEID=?',
					(request.form['dist'],request.form['gateid']))
		db.commit()
		flash('New distance has been seted ')
		return redirect(url_for('set_dist'))
	return render_template('setDistance.html',entries=entries)
...
def add_permission():
	...
	return render_template('add_permission.html',entries=entries)
